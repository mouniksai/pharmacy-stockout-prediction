...
* **AI-Assisted Scraping Workflow (Apify Web Scraper):** In alignment with course guidelines encouraging AI-assisted learning tools, **Apify Web Scraper** was utilized during initial exploratory crawling to inspect DOM schemas, pagination parameters, and public API response structures across Tata 1mg and Apollo Pharmacy. Insights gained from Apify's crawler workflows directly informed the architecture of our production-grade Python scraping engine (`src/web_scraper.py`).
...
* **AI-Assisted Preprocessing & Code Validation (Workik):** In accordance with course guidelines encouraging AI-assisted learning tools, **Workik** was utilized to generate baseline Pandas code structures for schema validation, missing value imputation checks, and datatype casting. In strict compliance with the rubric mandate requiring students to *"understand, validate, and appropriately modify the generated code before using it,"* the AI-assisted code was systematically reviewed, refactored, and significantly augmented: we replaced basic transformations with domain-specific inventory variables grounded in operations research ($DOI, LTD, \text{Buffer Ratio}, \text{Stock-to-Reorder Ratio}$) and integrated rigorous `scikit-learn` Pipeline abstractions (`ColumnTransformer`, `StandardScaler`, `OneHotEncoder(drop='first')`) to prevent data leakage.
